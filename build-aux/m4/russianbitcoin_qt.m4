...
dnl Output: If qt version is auto, set russianbitcoin_enable_qt to false. Else, exit.
AC_DEFUN([BITCOIN_QT_FAIL],[
  if test "x$russianbitcoin_qt_want_version" = xauto && test "x$russianbitcoin_qt_force" != xyes; then
    if test "x$russianbitcoin_enable_qt" != xno; then
      AC_MSG_WARN([$1; russianbitcoin-qt frontend will not be built])
    fi
    russianbitcoin_enable_qt=no
    russianbitcoin_enable_qt_test=no
  else
    AC_MSG_ERROR([$1])
  fi
])
...
AC_DEFUN([BITCOIN_QT_CHECK],[
  if test "x$russianbitcoin_enable_qt" != xno && test "x$russianbitcoin_qt_want_version" != xno; then
    true
    $1
  else
    true
...
AC_DEFUN([BITCOIN_QT_INIT],[
  ...
  AC_ARG_WITH([gui],
    ...
    [build russianbitcoin-qt GUI (default=auto)])],
    [
     russianbitcoin_qt_want_version=$withval
     if test "x$russianbitcoin_qt_want_version" = xyes; then
       russianbitcoin_qt_force=yes
       russianbitcoin_qt_want_version=auto
     fi
    ],
    [russianbitcoin_qt_want_version=auto])
...
dnl Outputs: russianbitcoin_enable_qt, russianbitcoin_enable_qt_dbus, russianbitcoin_enable_qt_test
AC_DEFUN([BITCOIN_QT_CONFIGURE],[
  ...
  BITCOIN_QT_CHECK([
    russianbitcoin_enable_qt=yes
    russianbitcoin_enable_qt_test=yes
    if test "x$have_qt_test" = xno; then
      russianbitcoin_enable_qt_test=no
    fi
    russianbitcoin_enable_qt_dbus=no
    if test "x$use_dbus" != xno && test "x$have_qt_dbus" = xyes; then
      russianbitcoin_enable_qt_dbus=yes
    fi
  ...
  if test x$russianbitcoin_enable_qt = xyes; then
    AC_MSG_RESULT([$russianbitcoin_enable_qt ($QT_LIB_PREFIX)])
  else
    AC_MSG_RESULT([$russianbitcoin_enable_qt])
  fi
...
dnl Inputs: russianbitcoin_qt_want_version (from --with-gui=). The version to check
dnl         first.
dnl Inputs: $1: If russianbitcoin_qt_want_version is "auto", check for this version
dnl         first.
...
dnl Inputs: russianbitcoin_qt_want_version (from --with-gui=). The version to use.
...
AC_DEFUN([_BITCOIN_QT_FIND_LIBS_WITHOUT_PKGCONFIG],[
  ...
  BITCOIN_QT_CHECK([
    if test "x$russianbitcoin_qt_want_version" = xauto; then
