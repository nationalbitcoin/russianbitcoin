...
double GetDifficulty(const CBlockIndex* blockindex)
{
    ...
    unsigned int nBits = GetNextBitsRequired(blockindex, &block, consensusParams);
...
double GetPoSKernelPS()
{
    ...
    nStakesTime = consensusParams.nTargetSpacing * nStakesHandled;
...
double GetEstimatedAnnualROI()
{
    ...
    double numberBlocksPerDay = 86400 / consensusParams.nTargetSpacing;
    if(networkWeight > 0)
    {
...
UniValue blockheaderToJSON(const CBlockIndex* tip, const CBlockIndex* blockindex)
{
    ...
    result.pushKV("chaintrust", blockindex->nChainTrust.GetHex());
    ...
    result.pushKV("flags", strprintf("%s", blockindex->IsProofOfStake()? "proof-of-stake" : "proof-of-authority"));
    if (blockindex->IsProofOfStake())
        result.pushKV("proofhash", blockindex->hashProof.GetHex());
    result.pushKV("signature", HexStr(blockindex->GetBlockHeader().GetSignature()));
    result.pushKV("signaturehash", HexStr(blockindex->GetBlockHeader().GetHashWithoutSign()));
...
UniValue blockToJSON(const CBlock& block, const CBlockIndex* tip, const CBlockIndex* blockindex, bool txDetails)
{
    ...
    result.pushKV("signaturehash", HexStr(block.GetHashWithoutSign()));
...
static RPCHelpMan getblock()
{
    return RPCHelpMan{"getblock",
                ...
                RPCResult::Type::OBJ, "", "",
                {
                    ...
                    {RPCResult::Type::STR_HEX, "bits", "The bits"},
                    {RPCResult::Type::NUM, "difficulty", "The difficulty"},
                    {RPCResult::Type::STR_HEX, "chaintrust", "Expected number of hashes required to produce the chain up to this block (in hex)"},
                    {RPCResult::Type::NUM, "nTx", "The number of transactions in the block"},
                    {RPCResult::Type::STR_HEX, "previousblockhash", "The hash of the previous block"},
                    {RPCResult::Type::STR_HEX, "nextblockhash", "The hash of the next block"},
                }},
    ...
    return RPCHelpMan{"getblockchaininfo",
                ...
                RPCResult{
                    RPCResult::Type::OBJ, "", "",
                    {
                        ...
                        {RPCResult::Type::STR_HEX, "chaintrust", "total amount of work in active chain, in hexadecimal"},
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    obj.pushKV("chaintrust",             tip->nChainTrust.GetHex());
