...
UniValue CreateColdStakeDelegation(const UniValue& params, CWallet* const pwallet)
{
    ...
    if (!IsValidDestination(stakeAddr))
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Russian Bitcoin address");
    ...
    if (params.size() > 1 && !params[1].isNull() && !params[1].get_str().empty()) {
        ...
        if (!IsValidDestination(dest))
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Russian Bitcoin spending address.");
...
static RPCHelpMan delegatestake()
{
    return RPCHelpMan{"delegatestake",
                ...
                HELP_REQUIRING_PASSPHRASE,
                {
                    {"staker_address", RPCArg::Type::STR, RPCArg::Optional::NO, "The Russian Bitcoin address to delegate to."},
                    {"owner_address", RPCArg::Type::STR, RPCArg::Optional::NO, "The Russian Bitcoin address corresponding to the key that will be able to spend the stake."},
