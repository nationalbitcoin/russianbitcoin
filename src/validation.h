...
bool CheckBlock(const CBlock& block, BlockValidationState& state, const Consensus::Params& consensusParams, bool fCheckPoA = true, bool fCheckMerkleRoot = true, bool fCheckSig=true);
...
bool TestBlockValidity(BlockValidationState& state, const CChainParams& chainparams, const CBlock& block, CBlockIndex* pindexPrev, bool fCheckPoA = true, bool fCheckMerkleRoot = true) EXCLUSIVE_LOCKS_REQUIRED(cs_main);
...
protected:
    ...
    /** chaintrust for the last block that preciousblock has been applied to. */
    arith_uint256 nLastPreciousChainTrust = 0;
...
public:
     ...
     *
     * Note that we guarantee that either the proof is valid on pblock, or
