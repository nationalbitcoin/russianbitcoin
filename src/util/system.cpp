...
const char * const BITCOIN_CONF_FILENAME = "russianbitcoin.conf";
...
#else
    const char* pszModule = "russianbitcoin";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Russian Bitcoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Russian Bitcoin
    // Mac: ~/Library/Application Support/Russian Bitcoin
    // Unix: ~/.russianbitcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Russian Bitcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Russian Bitcoin";
#else
    // Unix
    return pathRet / ".russianbitcoin";
#endif
#endif
}
