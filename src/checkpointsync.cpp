...
template<typename T>
static uint512 SignHash(const T& message)
{
...
bool AcceptPendingSyncCheckpoint()
{
...

    {
        ...
        if (g_rpc_node && !checkpointMessage.IsNull())
        {
...
bool CheckCheckpointPubKey()
{
    ...
    if (!strMasterPubKey.empty() && (!pblocktree->ReadCheckpointPubKey(strPubKey) || strPubKey != strMasterPubKey))
    {
...
bool SetCheckpointPrivKey(std::string strPrivKey)
{
    ...
    if (!key.Sign(SignHash(checkpoint.vchMsg), checkpoint.vchSig))
        return false;
    ...
    if (!pubkey.Verify(SignHash(checkpoint.vchMsg), checkpoint.vchSig))
        return false;
...
void CSyncCheckpoint::RelayTo(CNode* pfrom) const
{
    ...
    if (!strMasterPubKey.empty() && g_rpc_node && pfrom->hashCheckpointKnown != hashCheckpoint && pfrom->supportACPMessages)
    {
...
bool CSyncCheckpoint::CheckSignature()
{
    ...
    if (!strMasterPubKey.empty()) {
        ...
        if (!key.Verify(SignHash(vchMsg), vchSig))
